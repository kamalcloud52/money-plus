// Data Mockup Kategori Pengeluaran & Pemasukan
const expenseData = [
...
export function renderCategoryRanking() {
    ...
    const buildPieChart = (data) => {
        ...
        data.forEach((item) => {
            ...
            if (percentage < 0.1) return; // Skip jika 0%
    ...
    const renderList = (data) => {
        ...
        return data.map(item => {
            ...
            // Untuk styling warna teks (Merah untuk expense, Hijau untuk income)
            const colorClass = item.amount > 0 && data === expenseData ? 'text-red' : 'text-green';
            if (data === expenseData) { item.amount = -Math.abs(item.amount); } // Tampilkan minus untuk expense
                ...
                <div class="cr-item">
                    <div class="cr-header">
                        ...
                        <div class="cr-info">
                            ...
                            <span class="cr-amount ${colorClass}">${formatRupiah(item.amount)} (${pct.toFixed(1)}%)</span>
                        </div>
                    </div>
                    <div class="cr-bar-bg">
    ...
    return `
        <div class="app-container category-ranking-page">
            ...
            <div class="cr-total-row">
                <span>Total</span>
                <span class="fw-bold" id="cr-total-text">${formatRupiah(expenseData.reduce((a,b) => a + Math.abs(b.amount), 0))}</span>
            </div>

            <!-- List Kategori & Progress Bar -->
            ...
            <div class="bottom-sheet-content" style="max-height: 80vh; overflow-y: auto;">
                ...
                <div class="month-list">
                    ${['January','February','March','April','May','June','July','August','September','October','November','December'].map((m, i) => {
                        // Data dummy untuk bulan July saja
...
export function initCategoryRankingLogic() {
    ...
    // Tutup sheets jika klik area gelap
    ...
    // 3. Pilih Waktu (Update teks header)
    ...
    // 4. Tab Switch (Expense / Income) - Sederhana, hanya update UI (data mockup statis)
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            tabs.forEach(t => t.classList.remove('active'));
            tab.classList.add('active');
            // Logika switch warna dan data bisa ditambahkan di sini
        });
    });
}
